...
# Set Cipher and Encryption Settings
CIPHER="AES-256-GCM"
DATA_CIPHERS="AES-256-GCM:AES-128-GCM"
DATA_CIPHERS_FALLBACK="AES-256-CBC"

# Update server.conf generation
echo "cipher $CIPHER" >> /etc/openvpn/server.conf
echo "data-ciphers $DATA_CIPHERS" >> /etc/openvpn/server.conf
echo "data-ciphers-fallback $DATA_CIPHERS_FALLBACK" >> /etc/openvpn/server.conf

# Ensure ECDH is used for key exchange
echo "dh none" >> /etc/openvpn/server.conf
echo "ecdh-curve prime256v1" >> /etc/openvpn/server.conf

# Update client-template.txt accordingly
...
echo "Compression is not recommended. Do you still want to enable it?"
# Rest of the compression prompt code...
if [[ $COMPRESSION_ENABLED == "y" ]]; then
    echo "compress $COMPRESSION_ALG" >> /etc/openvpn/server.conf
...
function updateServerConfig() {
    ...
    # Specify the cipher and encryption settings
    local CIPHER="AES-256-GCM"
    local DATA_CIPHERS="AES-256-GCM:AES-128-GCM"
    local DATA_CIPHERS_FALLBACK="AES-256-CBC"

    # Modify or append the new configurations
    sed -i "/^cipher /c\cipher $CIPHER" /etc/openvpn/server.conf
    sed -i "/^data-ciphers /c\data-ciphers $DATA_CIPHERS" /etc/openvpn/server.conf
    sed -i "/^data-ciphers-fallback /c\data-ciphers-fallback $DATA_CIPHERS_FALLBACK" /etc/openvpn/server.conf
    sed -i "/^dh /c\dh none" /etc/openvpn/server.conf
    sed -i "/^ecdh-curve /c\ecdh-curve prime256v1" /etc/openvpn/server.conf
...
function upgradeOpenVPN() {
    echo "Upgrading OpenVPN to the latest version..."
    if [[ $OS == 'ubuntu' || $OS == 'debian' ]]; then
        # Import the public GPG key
        curl -fsSL https://swupdate.openvpn.net/repos/repo-public.gpg | gpg --dearmor > /etc/apt/keyrings/openvpn-repo-public.gpg
        mkdir -p /etc/apt/keyrings
        # Choose your desired OpenVPN version and distribution
        local ARCH="amd64" # or arm64, i386
        local VERSION="release/2.6" # Can be 'stable', 'testing', 'release/2.3', 'release/2.4', 'release/2.5', 'release/2.6'
        local OSRELEASE=$(lsb_release -sc) # For example, 'buster', 'focal', 'jammy'
        # Create sources.list for OpenVPN
    ...
    elif [[ $OS == 'centos' || $OS == 'amzn' || $OS == 'oracle' ]]; then
        # Use Fedora EPEL repository or Fedora Copr for latest versions
        yum install -y yum-plugin-copr
        yum copr enable -y dsommers/openvpn-release
        yum install -y openvpn
    elif [[ $OS == 'fedora' ]]; then
        # Fedora should have up-to-date OpenVPN in the main repositories
        dnf update -y openvpn
    elif [[ $OS == 'arch' ]]; then
        # Arch Linux usually has the latest version in its repositories
        pacman -Syu openvpn
    fi
    # Check if OpenVPN installed successfully
    if [[ $? -ne 0 ]]; then
        echo "Failed to install or upgrade OpenVPN. Please check your network or package manager."
        return 1
    fi
    ...
    echo "OpenVPN has been upgraded to the latest version."
}
...
# In the installOpenVPN() function, after the OpenVPN installation commands
